...
public class Controller {
    ...
    @PostMapping("/save")
    public ResponseEntity<?> saveVehicle(@RequestParam("file")MultipartFile file, @RequestParam("details") String details)
    {
        try{
            Vehicle vehicle=gson.fromJson(details,Vehicle.class);
            responseEntity = new ResponseEntity<>(vehicleService.registervehicle(vehicle,file.getBytes()), HttpStatus.OK);
        }
        catch (Exception e)
        {
